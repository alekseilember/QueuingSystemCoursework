...
namespace QueuingSystemCoursework
{
  internal class ExtractionManager
  {
    ...
    public LinkedList<(string, string, string, string, string, Request[], int?)> extractRequestAndPassToDevice(double systemTime, double alpha, double beta)
    {
      ...
      for (int i = 0; i < devices.Length; i++)
      {
        ...
        Statistics.RefusedRequestsCounter.ToString(), buffer, null));
          ...
          result.AddLast(devices[i].startService(request, systemTime, alpha, beta));
          return result;
        }
      }
